    ...
    useEffect(() => {
      ...
      async function like(e,s){
        ...
        const parse=JSON.parse(ud)
         const res=await axios.post('http://localhost:8080/like/addlike',{buyerID:parse._id,sellerID:s,propertyID:e,email:parse.email})
         const {message}=res.data
         if(message=='f'){
          toast.warn('Already Liked')
         }
         else if(message=='s'){
          const res2 = await axios.get('http://localhost:8080/property/sellerproperties');
