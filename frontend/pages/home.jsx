...
const Home = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing && currentPropertyId) {
        ...
        const res = await axios.put(`http://localhost:8080/property/properties/${currentPropertyId}`, {
          title,
          place,
          area,
          bedrooms,
  ...
  const handleDelete = async (id) => {
    try {
      const ud = localStorage.getItem('UserDetail');
      const parse = JSON.parse(ud);
      const res = await axios.delete(`http://localhost:8080/property/properties/${id}/${parse._id}`);
      const { message } = res.data;
      if (message === 's') {
        fetchProperties();
      }
  ...
  const interest = async (propertyId) => {
    ...
    const parse = JSON.parse(ud);
    try {
      const res = await axios.post('http://localhost:8080/like/getlike', {
        sellerID: parse._id,
